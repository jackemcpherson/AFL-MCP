"""HPN Player Approximate Value (PAV) calculation.

Implements the PAV formula created by HPN Footy, which produces a single
numeric value representing a player's total contribution across a season.
PAV combines team efficiency context with individual statistical output
across three zones: offence, midfield, and defence.

Available for seasons from 1998 onwards (inside 50s tracking began in 1998).
"""
...
target_season AS (
    ...
    WHERE s.year = %(year)s AND c.code = 'AFLM'
),
...
team_ratings AS (
    SELECT ts.team_id,
        (ts.pts_for::float / NULLIF(ts.i50_for, 0))
            / la.avg_pts_per_i50                    AS off_rating,
        ts.i50_for::float
            / NULLIF(ts.i50_against, 0)             AS mid_rating,
        (ts.pts_against::float / NULLIF(ts.i50_against, 0))
...
SELECT
    ...
    ts.season_id,
    pp.team_id,
    ROUND(pp.off_pav::numeric, 2),
    ROUND(pp.mid_pav::numeric, 2),
    ROUND(pp.def_pav::numeric, 2),
    ROUND((pp.off_pav + pp.mid_pav + pp.def_pav)::numeric, 2)
FROM player_pavs pp
...
def calculate_pav(year: int) -> int:
    ...
    Args:
        year: Season year (must be 1998 or later).

    Returns:
        Number of player-season-team rows upserted.

    Raises:
        ValueError: If year is before 1998 (inside 50s not tracked).
    """
    if year < MIN_PAV_YEAR:
        raise ValueError(
            f"PAV requires inside 50s data (available from {MIN_PAV_YEAR}). "
    ...
    with get_admin_connection() as conn:
        seasons = conn.execute(
            """SELECT DISTINCT s.year
               FROM seasons s
               JOIN competitions c ON s.competition_id = c.id
               WHERE c.code = 'AFLM' AND s.year >= %s
               ORDER BY s.year""",
            (MIN_PAV_YEAR,),
        ).fetchall()
