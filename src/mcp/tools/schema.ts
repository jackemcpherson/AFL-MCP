  ...
  return {
    database: {
      ...
      notes: [
        ...
        "match_lineups represents the post-change team — the players who actually took the field, with their on-field role at the start of the match. From 2023 onward this is sourced directly from the AFL API's post-change feed (~99.95% of non-emergency rows appear in stats). For 2021–2022 the AFL API only publishes the Thursday-night announced team, so those two seasons are derived from player_match_stats instead (every player with stats appears in lineups; guernsey number and position copied from the stats row). Emergencies (is_emergency=1) are named as alternates and typically do not play; this column is only populated from 2024 onward.",
