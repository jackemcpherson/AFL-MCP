...
function executeSQL(filePath: string): void {
  execSync(
    `npx wrangler d1 execute afl-stats --remote --file "${filePath}"`,
    { encoding: "utf-8", maxBuffer: 50 * 1024 * 1024 },
  )
}
...
async function main() {
  ...
  const candidates = queryD1<{
    keep_id: number; remove_id: number; first_name: string; surname: string;
  ...
  for (const c of candidates) {
    ...
    if (removeGroup.length === 1 && keepGroup.length === 1) {
      mergePairs.push({
        keepId: c.keep_id, removeId: c.remove_id,
        firstName: c.first_name, surname: c.surname,
    ...
    if (!removeTeams || removeTeams.size === 0) {
      ...
      skipped.push(`${c.first_name} ${c.surname} (keep=${c.keep_id}, remove=${c.remove_id}): ambiguous, no team data`)
      continue
    }

    if (!keepTeams || keepTeams.size === 0) {
      skipped.push(`${c.first_name} ${c.surname} (keep=${c.keep_id}, remove=${c.remove_id}): keep has no stats`)
      continue
    }
    ...
    if (overlap.length > 0) {
      ...
      skipped.push(`${c.first_name} ${c.surname} (keep=${c.keep_id}, remove=${c.remove_id}): no team overlap (keep=${[...keepTeams].join(",")}, remove=${[...removeTeams].join(",")})`)
    }
  }
  ...
  for (const pair of mergePairs) {
    ...
    stmts.push(
      `DELETE FROM players WHERE id = ${pair.removeId}`
    )
  ...
  // Step 5: Execute
  console.log("\nExecuting...")
  const sqlFiles = readdirSync(SQL_DIR).filter(f => f.startsWith("dedup_")).sort()
