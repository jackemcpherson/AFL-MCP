...
async function main() {
  ...
  const startYear = args.includes("--start") ? Number(args[args.indexOf("--start") + 1]) : MIN_LINEUP_YEAR
  const endYear = args.includes("--end") ? Number(args[args.indexOf("--end") + 1]) : new Date().getFullYear() - 1
  ...
  if (newPlayers.size > 0) {
    ...
    if (!dryRun) {
      ...
      for (const f of readdirSync(SQL_DIR).filter(f => f.startsWith("players_")).sort()) {
        console.log(`  ${f}`)
  ...
  for (const lineup of allLineups) {
    ...
    if (!matchId) { skippedMatches++; continue }
    ...
    for (const { players, teamId } of sides) {
      ...
      for (const p of players) {
        ...
        if (!playerId) { skippedPlayers++; continue }
  ...
  if (skippedMatches > 0) console.log(`Skipped ${skippedMatches} matches (no external_afl_id match)`)
  ...
  const lineupSqlFiles = readdirSync(SQL_DIR).filter(f => f.startsWith("lineups_")).sort()
