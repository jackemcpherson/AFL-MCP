...
async function main() {
  ...
  console.log(`Early lineup backfill: seasons ${startYear}-${endYear}${dryRun ? " (dry run)" : ""}`)
  ...
  const matchRows = queryD1<{
    id: number; year: number; round_number: number; home_team: string; external_afl_id: string | null
  }>(
  ...
  if (aflIdStmts.length > 0) {
    ...
    if (!dryRun) {
      ...
      for (const f of readdirSync(SQL_DIR).filter(f => f.startsWith("afl_ids_")).sort()) {
        console.log(`  ${f}`)
        executeSQL(join(SQL_DIR, f))
      }
    }
  ...
  for (const f of readdirSync(SQL_DIR).filter(f => f.startsWith("lineups_")).sort()) {
    console.log(`  ${f}`)
    executeSQL(join(SQL_DIR, f))
  }
