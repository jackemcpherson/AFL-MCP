...
export async function syncNewData(env: Env): Promise<void> {
  ...
  try {
    ...
    await env.DB.prepare(
      "INSERT OR IGNORE INTO competitions (code, name) VALUES (?, ?)"
    ).bind(COMPETITION_CODE, "AFL Men's").run()

    const competition = await env.DB.prepare(
      "SELECT id FROM competitions WHERE code = ?"
    ).bind(COMPETITION_CODE).first<{ id: number }>()
    ...
    await env.DB.prepare(
      "INSERT OR IGNORE INTO seasons (competition_id, year) VALUES (?, ?)"
    ).bind(competition.id, currentYear).run()

    const season = await env.DB.prepare(
      "SELECT id FROM seasons WHERE competition_id = ? AND year = ?"
    ).bind(competition.id, currentYear).first<{ id: number }>()
