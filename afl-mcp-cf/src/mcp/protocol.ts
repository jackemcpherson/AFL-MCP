...
export async function handleMcpRequest(
  ...
  if (request.method !== "POST") {
    return Response.json(
      jsonRpcError(0, -32600, "Only POST requests are accepted"),
      { status: 405 },
    )
  }
  ...
  if (!contentType.includes("application/json")) {
    return Response.json(
      jsonRpcError(0, -32700, "Content-Type must be application/json"),
      { status: 415 },
    )
  }
  ...
  try {
    ...
    return Response.json(
      jsonRpcError(0, -32700, "Parse error: invalid JSON"),
      { status: 400 },
    )
  }
