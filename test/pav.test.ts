...
describe("calculatePav", () => {
  it("rejects years before 1998", async () => {
    ...
    await expect(calculatePav(fakeEnv, 1997)).rejects.toThrow("PAV requires inside 50s data");
  });
  ...
  it("rejects the boundary year 1997 but accepts 1998", async () => {
    ...
    await expect(calculatePav(fakeEnv, 1998)).rejects.not.toThrow("PAV requires inside 50s data");
  });
});
