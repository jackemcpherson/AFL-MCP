...
for (const table of TABLES) {
  ...
  const status = match ? "OK" : `MISMATCH (diff: ${d1Count - csvCount})`;
  console.log(table.padEnd(25), String(csvCount).padEnd(10), String(d1Count).padEnd(10), status);
}
...
console.log("2024 Grand Final:", JSON.parse(matchCheck)[0]?.results?.[0] ? "Found" : "Missing");
