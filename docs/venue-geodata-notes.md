...
**Date:** 2026-07-13
**Dataset:** [`data/venue-geodata.csv`](../data/venue-geodata.csv) (106 rows, one per D1 venue id)
...
an ~11 km reanalysis grid, so the precision bar for coordinates is low —
within ~3 km of the ground is sufficient. Coordinates are decimal degrees at
4 dp.
...
- Obscure and sponsor-named grounds (mostly VFL/VFLW/AFLW venues 2017+) were
  verified via web search against Austadiums, Wikipedia, AFL.com.au, club
  sites and council pages. Rows verified this way carry a source note in the
  CSV `notes` column.
...
- `roof` is `retractable` for Marvel Stadium (Docklands) only — no other
  venue in the list has a roof. The placeholder row has all geodata fields
  empty.
- ACT venues (Manuka Oval, Bruce Stadium) use `Australia/Sydney`.

Sanity checks run before committing: all 106 D1 venue ids present exactly
once; every non-placeholder row has coords + timezone; timezone consistent
with longitude/state; every canonical target is the highest-match-count row
in its alias group; alias rows share coordinates with their canonical row.

## 2. Alias Table

12 alias groups (alias → canonical, canonical = highest match count for the
...
Numbers in parentheses are D1 match counts. All other rows are
self-canonical.
...
Richmond, **South Australia** (West Adelaide FC; Adelaide Crows' 2020 AFLW
home ground) — not Punt Road Oval in Melbourne. Verified via Wikipedia, so
it carries `high` confidence, but flagged here because the name invites
confusion.
...
55 matches in production D1 are assigned to the placeholder venue
"To Be Confirmed". These matches **cannot be weather-backfilled** from this
dataset — they have no coordinates or timezone. Before (or as part of) the
weather backfill, these matches should be re-resolved to real venues from the
source fixture data; until then, expect 55 matches with NULL weather after
